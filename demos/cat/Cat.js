...
const Cat = (function(){
  ...
  // private vars:
  let _domVideo = null, _domCanvas = null, _domContainer = null, _domMarker = null;
  ...
  function load_neuralNet(){
    WEBARROCKSOBJECT.set_NN(_settings.neuralNet, function(errLabel){
      if (errLabel){
  ...
  function start(){
    ...
    _domMarker.setAttribute('class', 'CatMarker');
  ...
  function on_videoReady(){
    if (!_domVideo.videoWidth || !_domVideo.videoHeight){
      console.log('WARNING in Cat: the video is not ready. ');
    ...
    console.log('INFO in Cat: the video is ready :)');
  ...
  function on_videoEnd(){
    console.log('INFO in Cat: video ended, restart it');
    _domVideo.play();
    WEBARROCKSOBJECT.reset_state();
  }
  ...
  function iterate(){ // detect loop
    ...
    const detectState = WEBARROCKSOBJECT.detect(_settings.nDetectionsPerLoop, null, _detectOptions);
  ...
  const that = {
    init: function(spec){
      if (_state !== _states.notLoaded){
        console.log('WARNING in Cat: main() should be called once');
        return;
      }

      if (check_isMobile() || screen.availWidth<1260){
        console.log('WARNING in Cat: mobile detected, abort');
      }
      ...
      _domVideo = create_domDisplay('video', 'CatDisplay');
      _domVideo.loop = true;
      ...
      // otherwise it won't play with shitty browsers like Safari
