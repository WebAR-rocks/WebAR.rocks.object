...
const WebARRocksWebXRHelper = (function(){
  // settings:
  const _settings = {
    isIOScolorProcessing: false, //process IOS video with color. not advised
    IOSthresholdDetectFactor: {
      'NN_OBJ4_0.json': 1,
      'NN_OBJ4LIGHT_0.json': 0.42 // should lower the threshold because input size of the NN (128px)
  ...
  let _detectState = null;
  let _nDetections = 2;
  let _scaleFactor = _settings.scaleFactor;
  let _visionDoneCallback = null;
  const _pose = {
    x:0,y:0,
    yaw:0,
    scale:1
  ...
  let _IOSthresholdDetectFactor = 1;
  ...
  function convert_YCbCr2rgba32Color(YFrame, CbCrFrame, outFrame){ // for IOS
    ...
    for (const outY=0; outY<outHeight; ++outY){
      for (const outX=0; outX<outWidth; ++outX){

        // compute the RGBA32 of outFrame buffer (outX, outY):
        // get the lum value:
        const lumX = Math.round(outX*YFrame.size.width/outWidth);
        const lumY = Math.round(outY*YFrame.size.height/outHeight);
        const lum = YFrame.buffer[lumY*YFrame.size.bytesPerRow + lumX*YFrame.size.bytesPerPixel];
        ...
        const p = outY*CbCrFrame.size.bytesPerRow + outX*CbCrFrame.size.bytesPerPixel ;
        let cb = CbCrFrame.buffer[p++];
        let cr = CbCrFrame.buffer[p];

        // luv -> rgb.  see https://www.fourcc.org/fccyvrgb.php
        const y = 1.1643*(lum-16);
        const u = cb - 128;
        const v = cr - 128;
        cr = clamp(y+1.5958*v,      0, 255);
        let cg = clamp(y-0.39173*u-0.81290*v, 0, 255);
        cb = clamp(y+2.017*u,       0, 255); 

        // write the result:
        //outN=4*(outX+outFrame.size.width*outY);
        const outN = 4*((outHeight-1-outY)+outFrame.size.width*outX);
  ...
  function convert_YCbCr2rgba32GrayScale(YFrame, outFrame){
    ...
    for (const outY=0; outY<outHeight; ++outY){
      for (const outX=0; outX<outWidth; ++outX){
        ...
        // get the lum value:
        const lum = YFrame.buffer[outY*YFrame.size.bytesPerRow + outX*YFrame.size.bytesPerPixel];
  ...
  const that = {
    init: function(spec){ //initialization
      if (_state!==_states.notLoaded){
        console.log('ERROR in WebARRocksWebXRHelper: cannot initialize multiple times. Abort');
        return;
      }
      _state = _states.loading;

      const WebARRocksCanvas = document.createElement('canvas');
      if (spec.DOMCanvasParent){
        spec.DOMCanvasParent.appendChild(WebARRocksCanvas);
        WebARRocksCanvas.className = 'WebARRocksCanvas';
      }
      ...
      console.log('INFO in WebARRocksWebXRHelper.js: init WEBARROCKSOBJECT...');
      WEBARROCKSOBJECT.init({
        canvas: WebARRocksCanvas,
        isDebugRender: spec.isDebugRender,
        callbackReady: function(errLabel){
          //DebugUtils.log_domConsole('INFO in WebARRocksWebXRHelper.js: WEBARROCKSOBJECT callbackReady launched with errLabel='+errLabel.toString());
          if (errLabel){
            _state=_states.error;
            console.log('ERROR in WebARRocksWebXRHelper.js - cannot init WebAR.rocks.object: ', errLabel);
          } else {
    ...
    load_neuralNet: function(NNPath){
      if (_state===_states.error){
        return;
      }
      _state=_states.loading;
      WEBARROCKSOBJECT.set_NN(NNPath, function(errLabel){
        if (errLabel){
          console.log('INFO in WebARRocksWebXRHelper.js: cannot load neural net ', NNPath);
        } else {
          console.log('INFO in WebARRocksWebXRHelper.js: ready for detection :)');
          _state=_states.idle;
        }
      });
    },
    ...
    videoWorker: function(event){
      ...
      if (!event || !event.type){
        console.log('WARNING in WebARRocksWebXRHelper: INVALID EVENT. event =', event);
      }
      ...
      switch(event.type){
        case 'videoFrame':
          ...
          let videoFrameBufferRGBA32 = null;
          ...
          _detectState = WEBARROCKSOBJECT.detect(_nDetections, videoFrameBufferRGBA32, _detectOptions);
        ...
        default:
          console.log('WARNING in WebARRocksWebXRHelper: unknow event type received ', event.type);
          break;
    ...
    extract_lookUpDownAngleFromCamera: function(threeCamera){ //0 -> camera looks horizontally, PI/2 -> vertically (down)
      ...
      let cameraEulerX = angleMod(threeCameraEuler.x); 
    ...
    compute_pose: function(detectState, threeCamera){
      ...
      const detectCameraAspect = WEBARROCKSOBJECT.get_aspectRatio();
    ...
    update_pose: function(threeMatrixAnchoredWorld, threeCamera, pose){
      ...
      const threePositionAnchoredWorld = new THREE.Vector3().setFromMatrixPosition(threeMatrixAnchoredWorld);
